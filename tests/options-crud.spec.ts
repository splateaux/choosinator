...
test.describe("Options CRUD", () => {
    test("owners and editors can add/edit/delete options; viewers cannot", async ({
        ...
        await expect(page.getByText(`List shared with ${editorEmail}`)).toBeVisible();

        await page.getByLabel("Share with (email address)").fill(viewerEmail);
        await page.getByLabel(/permission/i).selectOption("view");
        await page.getByRole("button", { name: "Share" }).click();
        await expect(page.getByText(`List shared with ${viewerEmail}`)).toBeVisible();
        ...
        const editNameInput = page.locator('details form input[name="name"]').first();
        ...
        await page.waitForURL(/\/optionsLists(\/)?$/);
        const editorSharedLink = page.getByRole("link", { name: new RegExp(listName) });
        ...
        await page.getByRole("button", { name: /log in/i }).click();
        await page.waitForURL(/\/optionsLists(\/)?$/);
        const viewerSharedLink = page.getByRole("link", { name: new RegExp(listName) });
        ...
        await expect(page.getByTestId("options-view-only")).toHaveText(/view only/i);
        await expect(page.getByRole("button", { name: /add option/i })).toHaveCount(0);
