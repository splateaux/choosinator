...
describe("Session Server", () => {
  ...
  beforeEach(async () => {
    ...
    vi.spyOn(sessionStorage, "commitSession").mockResolvedValue(
      "committed-session",
    );
    vi.spyOn(sessionStorage, "destroySession").mockResolvedValue(
      "destroyed-session",
    );
  });
  ...
  describe("getUser", () => {
    it("should return user when session exists and user is found", async () => {
      ...
      (getUserById as MockedFunction<typeof getUserById>).mockResolvedValue(
        mockUser,
      );
    ...
    it("should throw logout redirect when user not found", async () => {
      ...
      const mockLogoutResponse = {
        url: "/",
        init: { headers: { "Set-Cookie": "destroyed-session" } },
      };
      const remix = await import("@remix-run/node");
      (remix.redirect as MockedFunction<typeof remix.redirect>).mockReturnValue(
        mockLogoutResponse as any,
      );
