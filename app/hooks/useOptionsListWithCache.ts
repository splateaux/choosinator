...
import { OptionsListCache, PersistentOptionsListCache } from '~/utils/optionsListCache';
...
export function useOptionsListWithCache(
    ...
    useEffect(() => {
        ...
        if (initialData) {
            ...
            setState(prev => ({ ...prev, optionsList: initialData, isCached: false }));
            return;
        }

        // Try to get from cache first
        const cached = OptionsListCache.get(optionsListId) ||
...
export async function preloadOptionsList(id: string, userId: string): Promise<void> {
