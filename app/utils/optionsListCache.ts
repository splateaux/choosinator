...
export class OptionsListCache {
    ...
    static get(id: string): OptionsList | null {
        ...
        console.log(`📋 Cache HIT for options list: ${id} (${Date.now() - entry.timestamp}ms old)`);
        return entry.data;
    }
    ...
    private static cleanup(): void {
        ...
        if (cleaned > 0) {
            console.log(`🧹 Cleaned ${cleaned} expired entries from options list cache`);
        }
    }
}
...
export class PersistentOptionsListCache {
    ...
    private static getStoredCache(): Record<string, { data: OptionsList; timestamp: number }> {
        try {
